...
try {
    ...
    // 3. Prepare Variables (MySQLi requires variables for bind_param)
    ...
    // NPS/Overall Satisfaction Rating from Section 3
    $overall_rating     = nullify($_POST['overall_rating'] ?? null);

    // 4. SQL Statement (MySQLi uses ? placeholders)
    ...
    // 5. Bind Parameters
    // "s" means string, "i" means integer. Using "s" for almost all to handle nulls/strings safely.
    // There are 32 placeholders total.
    $stmt->bind_param(
    ...
    if ($stmt->execute()) {
        // 6. Success - Redirect to the Thank You UI page
        $stmt->close();
        $conn->close();
